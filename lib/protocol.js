...
// The ten-hour test edition (contracts from script/Deploy.s.sol:DeployTest): a separate build at /test with a banner on
// every page and times shown to the minute.
...
// Batch claims and withdrawals (requestClaimMany / withdrawMany) exist only in the contracts deployed from this
// repo's current source. The editions live today predate them, so the screens fall back to one transaction at a time
// unless a build says otherwise.
export const BATCH_CLAIMS = process.env.NEXT_PUBLIC_BATCH_CLAIMS === '1';
