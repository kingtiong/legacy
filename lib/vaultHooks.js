...
import { TEST_MODE, VAULT_ADDRESS, MARKET_ADDRESS, CHAIN_ID, FEE_SHARES_ID, shareId, cohortOf, bucketOf } from './protocol';
...
export function usePositions(account, stats) {
  ...
  // One call per 250 ids: a test edition mints a cohort every five minutes, and a single call asking about every
  // cohort ever opened grows without limit until public RPCs refuse it.
  const chunks = useMemo(() => {
    const out = [];
    for (let i = 0; i < ids.length; i += 250) out.push(ids.slice(i, i + 250));
    return out;
  }, [ids]);

  const balances = useReadContracts({
    contracts: account
      ? chunks.map((c) => ({ ...vault, functionName: 'balanceOfBatch', args: [c.map(() => account), c] }))
      : [],
    query: { enabled: Boolean(account && chunks.length), refetchInterval: 30_000 },
  });
  ...
  const held = useMemo(() => {
    ...
    const out = [];
    chunks.forEach((c, ci) => {
      const r = balances.data[ci];
      if (r?.status !== 'success') return;
      c.forEach((id, i) => {
        if (r.result[i] > 0n) out.push({ id, shares: r.result[i] });
      });
    });
    return out;
  }, [balances.data, chunks]);
...
/** Test edition: cohorts last minutes, so show the local date and time instead. */
const minute = (ts) =>
  new Date(ts * 1000).toLocaleString('en-GB', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' });

export function day(ts) {
  if (TEST_MODE) return minute(ts);
